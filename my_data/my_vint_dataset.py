...
class ViNT_Dataset_BC(Dataset):
    def __init__(
        ...
        Args:
            ...
            min_action_distance (int): Minimum distance for action to use
            max_action_distance (int): Maximum distance for action to use
    ...
    def _build_index(self, use_tqdm: bool = False):
        ...
        for traj_name in tqdm.tqdm(self.traj_names, disable=not use_tqdm, dynamic_ncols=True):
            ...
            end_time = traj_len - self.len_traj_pred * 2
    ...
    def _compute_actions(self, traj_data, curr_time):
        ...
        end_index = curr_time + self.len_traj_pred * 2 + 1
        pos = traj_data[["pos_x(m)", "pos_y(m)"]][start_index:end_index:2].to_numpy(dtype=np.float64)
...
class ViNT_Dataset_GOAL(Dataset):
    def __init__(
        self,
        data_folder: str,
        data_split_folder: str,
        ...
        Args:
            ...
            normalize (bool): Whether to normalize the distances or actions
            obs_type (str): What data type to use for the observation
            goal_type (str): What data type to use for the goal
        """
        self.data_folder = data_folder
        self.data_split_folder = data_split_folder
        self.dataset_name = dataset_name
        ...
        with open(
            os.path.join(os.path.dirname(__file__), "my_data_config.yaml"), "r"
        ) as f:
            all_data_config = yaml.safe_load(f)
        assert (
            self.dataset_name in all_data_config
        ), f"Dataset {self.dataset_name} not found in data_config.yaml"
        self.data_config = all_data_config["ourdata"]
    ...
    def _build_caches(self, use_tqdm: bool = True):
        ...
        cache_filename = os.path.join(
            self.data_split_folder,
            f"my_{self.dataset_name}_images.lmdb",
        )
    ...
    def __getitem__(self, i: int) -> Tuple[torch.Tensor]:
        ...
        Args:
            i (int): index to ith datapoint
        Returns:
            Tuple of tensors containing the context, observation, goal and action label
                obs_image (torch.Tensor): tensor of shape [3, H, W] containing the image of the robot's observation
        ...
        obs_times = [curr_time + i for i in range(-self.context_size, 1)]
        obs_context = [(f_curr, t) for t in obs_times]
        goal_times = [curr_time + i for i in range(1, self.len_traj_pred + 1)]
        goal_context = [(f_curr, t) for t in goal_times]

        obs_image = torch.cat([
            self._load_image(f, t) for f, t in obs_context
        ])
        goal_image = torch.cat([
            self._load_image(f, t) for f, t in goal_context
        ])
