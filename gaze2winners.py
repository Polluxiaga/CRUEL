...
def extract_peak_fixations_from_gaze_maps(pt_file_path: str) -> dict:
    ...
    for key, gaze_map_tensor in loaded_gaze_maps_individual.items():
        ...
        if gaze_map_2d.dim() != 2:
            print(f"Warning: Gaze map for key {key} in '{os.ado.basename(pt_file_path)}' has unexpected dimensions {gaze_map_2d.shape}. Skipping peak extraction.")
...
def extract_and_save_all_selected_ids(root_folder: str, 
    ...
    if all_selected_ids:
        ...
        print(f"No selected IDs were generated for this split. Final .pt file not created for {output_pt_file_path}.")
